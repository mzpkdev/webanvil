...
/** webanvil's default compilerOptions, reused as fixture input for the tsconfig writers. */
...
describe("tools", () => {
    ...
    context("scaffoldTsconfig", () => {
        it("writes webanvil's default tsconfig into the given dir with root-relative globs", async () => {
            const dir = mkdtempSync(path.join(tmpdir(), "webanvil-scaffold-"))
    ...
    context("ownedTsconfig", () => {
        it("writes a headered, flat tsconfig from the merged options", async () => {
            const dir = mkdtempSync(path.join(tmpdir(), "webanvil-owned-"))
            const file = await ownedTsconfig(OPTS, false, dir)
            expect(file).toBe(path.join(dir, "tsconfig.json"))
            const text = readFileSync(file, "utf8")
            expect(text.startsWith("// Generated by webanvil")).toBe(true)
    ...
    context("resolveTurboConfig", () => {
        ...
        it("scaffolds a default (unheadered) turbo.json when there is no webanvil.config", async () => {
            const dir = mkdtempSync(path.join(tmpdir(), "webanvil-turbo-"))
        ...
        it("owns a headered turbo.json when a webanvil.config exists", async () => {
            const dir = mkdtempSync(path.join(tmpdir(), "webanvil-turbo-"))
            writeFileSync(path.join(dir, "webanvil.config.json"), "{}")
            await resolveTurboConfig(TASKS, dir)
            expect(readFileSync(path.join(dir, "turbo.json"), "utf8").startsWith("// Generated by webanvil")).toBe(true)
        })

        it("leaves an existing turbo.json untouched without a webanvil.config", async () => {
            const dir = mkdtempSync(path.join(tmpdir(), "webanvil-turbo-"))
    ...
    context("run", () => {
        ...
        it("spawns the resolved bin under webanvil's runtime with the given args", async () => {
    ...
    context("detectConfig", () => {
        // Runs in webanvil's own package dir, which has tsconfig.json, vitest.config.ts,
