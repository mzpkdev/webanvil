...
export type { ResolvedConfig, Target, WebanvilConfig } from "./config"
...
export function main(argv: string[] = process.argv.slice(2)): Promise<number> {
    ...
    // is parsed by cmdore as webanvil's own command, arguments, and options.
    const cut = argv.indexOf("--")
    const own = cut === -1 ? argv : argv.slice(0, cut)
    setPassthrough(cut === -1 ? [] : argv.slice(cut + 1))
    // Record which flags were explicitly typed so commands can layer them over webanvil.config.
