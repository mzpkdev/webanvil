...
import { hasVialConfig, loadVialConfig } from "../config"
...
export const typecheckCommand = defineCommand({
    ...
    run: async ({ watch, config, "no-scaffold": skipScaffold, check }) => {
        const { typecheck } = await loadVialConfig()
        ...
        // it by walking up from source. When a vial.config drives the project, vial owns and
