...
`--port` on `wa dev --storybook` configure Storybook. The package build still
owns `--out-dir`.
...
| `wa dev [entry]`          | Starts Vite or a Node build watcher. `--storybook` also starts the configured Storybook.                   | `--mode`, `--out-dir`, `--host`, `--port`, `--copy`, `--bundle`, `--no-bundle`, `--formats`, `--declaration`, `--sourcemap`, `--minify`, `--platform`, `--target`, `--storybook` |
